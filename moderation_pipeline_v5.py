...
class SupervisorAgent:
    ...
    def plan_and_moderate(self, image_path):
        ...
        # Normalize outputs to sane defaults
        crops = [] if crops is None else list(crops)
        boxes = [] if boxes is None else list(boxes)
        nsfw_flags = [] if nsfw_flags is None else list(nsfw_flags)

        # Hardening: keep arrays in sync
        if len(nsfw_flags) < len(crops):
            nsfw_flags += [None] * (len(crops) - len(nsfw_flags))
        if len(boxes) < len(crops):
            # If detector didn’t return boxes for every crop, pad with placeholders
            boxes += ["unknown"] * (len(crops) - len(boxes))

        print(f"🎬 SUPERVISOR: Detector returned {len(crops)} region(s)")
        ...
        # If detector found nothing, still moderate the whole image
        if len(crops) == 0:
            print("\n🎬 SUPERVISOR: No avatars detected. Running whole-image moderation fallback...")
            nsfw_label, nsfw_prob = self.nsfw_agent.act(frame, None)
            results["regions"].append({
                "box": "whole_image",
                "nsfw_label": nsfw_label,
                "nsfw_prob": float(nsfw_prob)
            })
            print(f"🎬 SUPERVISOR: Whole-image assessment complete - {nsfw_label}")
        else:
            # Process regions - Pass nsfw_flag to NSFW agent
            print(f"\n🎬 SUPERVISOR: Processing {len(crops)} detected region(s)...")
...
if __name__ == "__main__":
    ...
    moderate_images(r"in-the-wild/nsfw/nsfw_1", num_images=-1)